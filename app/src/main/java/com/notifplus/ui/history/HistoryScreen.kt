...
fun HistoryScreen(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        LazyColumn(
            ...
            item {
                AccessBanner(accessGranted, listenerHealth, onRequestAccess, onRequestRebind)
            }
            item {
                OutlinedTextField(
                ...
                ExposedDropdownMenuBox(
                    ...
                    DropdownMenu(expanded = filterExpanded, onDismissRequest = { filterExpanded = false }) {
                        DropdownMenuItem(
                            ...
                            onClick = { viewModel.setPackageFilter(null); filterExpanded = false },
                        )
                        knownPackages.forEach { packageName ->
                            DropdownMenuItem(
                                text = { Text(packageName) },
                                onClick = { viewModel.setPackageFilter(packageName); filterExpanded = false },
                            )
                        }
                    }
                }
            ...
            if (notifications.itemCount == 0 && notifications.loadState.refresh is LoadState.NotLoading) {
                item {
                    EmptyState(
                        ...
                        message = "You have no new notifications matching the filter."
                    )
                }
            } else {
